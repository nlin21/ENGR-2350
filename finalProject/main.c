...
uint16_t desired_speed = 200;
uint16_t desired_speed_L, desired_speed_R, differential_speed;
uint16_t current_speed_L, current_speed_R;
...
int main(void)
{
    ...
    while(1){
        if (run_control) {
            ...
            printf("Ranger Value: %u\r\n", readRanger());


            if (range < 20) {
                desired_speed = 100;
                differential_speed = desired_speed * (0.5 * (DISTANCE_BETWEEN_WHEELS_IN_MM / 25.4) / (DISTANCE_BETWEEN_WHEELS_IN_MM / 25.4 / 2.0));
